...
EEPROM_STATUS EEPROM_93C86_getStatus(void) {
  EEPROM_93C86_CS_SELECT();
  EEPROM_STATUS status = (HAL_GPIO_ReadPin(SPI_MISO_GPIO_Port, SPI_MISO_Pin) == GPIO_PIN_RESET) ? EEPROM_BUSY : EEPROM_OK;
  EEPROM_93C86_CS_UNSELECT();
  return status;
}
...
EEPROM_STATUS EEPROM_93C86_ReadMultipleWords(uint16_t startAddress, uint16_t* buffer, uint16_t length) {
  EEPROM_STATUS status = EEPROM_OK;
  for (uint16_t i = 0; i < length; i++) {
    status = EEPROM_93C86_Read(startAddress + i, &buffer[i]);
    if (status != EEPROM_OK) return status; // Return immediately if any read operation fails
  }

  return status; // Return OK if all read operations were successful
}

EEPROM_STATUS EEPROM_93C86_WriteMultipleWords(uint16_t startAddress, uint16_t* buffer, uint16_t length) {
  EEPROM_STATUS status = EEPROM_OK;
  for (uint16_t i = 0; i < length; i++) {
