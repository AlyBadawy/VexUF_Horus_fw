...
uint16_t getSerialBytes(void) { return *(uint16_t *)0x08007FF0; }
...
void base32_encode(const uint8_t *data, size_t length, char *output) {
  ...
  for (i = 0; i < length; i++) {
    ...
    while (bits_in_buffer >= 5) {
      output[index++] =
          custom_base32_alphabet[(bitbuffer >> (bits_in_buffer - 5)) & 0x1F];
      bits_in_buffer -= 5;
    }
  }
  ...
  if (bits_in_buffer > 0) {
    output[index++] =
        custom_base32_alphabet[(bitbuffer << (5 - bits_in_buffer)) & 0x1F];
  }
